...
if __name__ == "__main__":
    ...
    # read real faces
    root = '/home/na/1_Face_morphing/2_data/1_self_collect/AA_real_raw_v2/'
    real_path = root + '3_raw_aligned_1024_rename_V2/'
    dst_path = root + '3_raw_aligned_1024_rename_V2_percept/'
    dst_latent = root + '3_raw_aligned_1024_rename_V2_percept_latent/'
    folds = ['female', 'male']
    for fold in folds:
        if os.path.exists(dst_path + fold) is False:
            os.makedirs(dst_path + fold)
        if os.path.exists(dst_latent + fold) is False:
            os.makedirs(dst_latent + fold)

        img_list = os.listdir(real_path + fold + '/')
        for img in img_list:
            path_img1 = real_path + fold + '/' + img
            w1 = projection(args, path_img1, percept, G, latent_mean, latent_std)
            dd = w1.detach().cpu().numpy()
            data = {}
            data['w'] = dd
            sio.savemat(dst_latent + fold + '/' + img.split('.')[0] + '.mat', data)


            # # Generate an image
            imgs = G(w1, args.truncation_psi)[0].cpu().numpy()
            dst_img = dst_path + fold + '/' + img
            imgd = crop(misc.to_pil(imgs[0]), args.ratio).save(dst_img)
