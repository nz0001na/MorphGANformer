...
def projection(args, path_img1, AWingLoss, G, latent_mean, latent_std, detector, predictor):
    ...
    for i in pbar:
        ...
        loss = 0.00001 * AWingLoss(y_hat, y)
        # loss.backward()
        ...
        num_loss = loss.detach().cpu().numpy()
        ...
        pbar.set_description(
            (
                f" AdaptiveWingLoss: {loss.item():.4f};"
                # f" WingLoss: {float(num_loss):.4f};"
                f" min_loss: {min_loss:.4f}; lr: {lr:.4f}"
            )
        )
...
if __name__ == "__main__":
    ...
    output_dir = 'images/2_frgc_data/frgc_exp_1024/wing_loss_adaptive/'
    if os.path.exists(output_dir) is False:
        os.makedirs(output_dir)
    ...
    AWingLoss = adaptive_wing_loss.AdaptiveWingLoss()

    path_img1 = 'images/2_frgc_data/frgc_exp_1024/04827d02.png'
    w1 = projection(args, path_img1, AWingLoss, G, latent_mean, latent_std, detector, predictor)