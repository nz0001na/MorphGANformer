...
def projection(args, path_img1, percept, G, latent_mean, latent_std):
    ...
    for i in pbar:
        ...
        if num_loss < min_loss:
            ...
            # Save the image
            output_dir = 'images/example/rand_pert_im1/'
...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        ...
        mean = noise_sample.mean(0)
        latent_mean = torch.randn(1, *G.input_shape[1:], device=device)
        latent_mean = latent_mean.mean(0)

        latent_std = ((noise_sample - mean).pow(2).sum() / args.n_mean_latent) ** 0.5
    ...
    path_img1 = 'images/example/1.png'
    ...
    dst_img = 'images/example/rand_v1_morph01.png'
