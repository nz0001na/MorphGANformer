...
def projection(args, path_img1, G, latent_mean, latent_std):
    ...
    for i in pbar:
        ...
        psnr_loss = psnr(pre.flatten(), trg.flatten())
        psnr_loss = torch.FloatTensor(np.array(psnr_loss))
        psnr_loss = psnr_loss.to(torch.device("cuda"))
        # psnr_loss = np2tensor(psnr_loss).to(torch.device("cuda"))
        psnr_loss.requires_grad = True
        ...
        num_loss = psnr_loss.detach().cpu().numpy()
...
if __name__ == "__main__":
    ...
    out_dir = 'images/2_frgc_data/frgc_exp_1024/psnr'
