...
def setup_config(run_dir, **args):
    ...
    # if args.kgan and args.transformer:
        # img.error("Either have --transformer for GANformer or --kgan for k-GAN, not both")
...
def main():
    ...
    #
    ro = '/home/na/1_Face_morphing/1_code/2_morphing/5_gansformer-main_V2_256/pytorch_version/images'
    data_dir = ro
    dataset = '2_raw_tf'
    ## Dataset
    # parser.add_argument("--data-dir",           help = "Datasets root directory (default: %(default)s)", default = "datasets", metavar = "DIR")
    parser.add_argument("--data-dir", help="Datasets root directory (default: %(default)s)", default=data_dir,
                        metavar="DIR")
    parser.add_argument("--dataset",            help = "Training dataset name (subdirectory of data-dir)", default = dataset) # , required = True
